...
app.post('/upload', upload.single('file'), async (req,res)=>{
  try{
    ...
    console.error(e); res.status(500).json({ error:'Falha ao criar job' });
  }
});
...
app.get('/status/:id', async (req,res)=>{
  try{
    ...
    }else if(state==='failed'){ error = job.failedReason || 'Falha'; }

    res.json({ id:job.id, state, outputUrl, error });
  }catch(e){
    console.error(e); res.status(500).json({ error:'Erro ao checar status' });
  }
});
...
app.get('/download/:id', async (req,res)=>{
  try{
    ...
    if(!job) return res.status(404).json({ error:'Job não encontrado' });
    const ext = job.data.target==='pdf'?'pdf':job.data.target==='csv'?'csv':'webp';
    const filePath = jobOutputPath(job.id, ext);
    if(!filePath.startsWith(OUTPUT_DIR)) return res.status(400).json({ error:'Caminho inválido' });
    const name = `${path.basename(job.data.originalName, path.extname(job.data.originalName))}.${ext}`;
    res.download(filePath, name);
  }catch(e){
    console.error(e); res.status(500).json({ error:'Erro no download' });
  }
});
